...
export class SourceObjectSectionView extends BreakoutSectionView {
    ...
    /**
     * Creates a new {@link SourceObjectSectionView}.
     * @param file
    ...
    constructor(file: MappingFileView, objectId: string | null, objectText: string | null) {
        const name = objectId === null ? "No Source" : `${ objectId }: ${ objectText }`;
    ...
    public applySectionValue(obj: MappingObject): EditorCommand {
        ...
        if(this.value && !obj.sourceObject.framework.has(this.value, this.objectText)) {
            return EditorCommands.setFrameworkObjectPropertyId(obj.sourceObject, this.value, this.objectText);
        } else {
            return EditorCommands.setFrameworkObjectPropertyId(obj.sourceObject, this.value);
        }   
    }
